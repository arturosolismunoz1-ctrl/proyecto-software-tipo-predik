...
import {
  ...
  apiGenerarReporte, apiBdStatus, apiPreviewReporte,
} from '../api/client'
import type { PreviewData } from '../api/client'
import type {
  Capa, ColorNombre, EstadoCatalogo, MunicipioCatalogo,
  MunicipioBbox, GeoJSONGeometry, Clasificacion, Formato, NivelGeografico,
} from '../types'
...
interface LayerRowProps {
  capa: Capa; estados: EstadoCatalogo[]
  onChange: (c: Capa) => void; onRemove: () => void
  canRemove: boolean; index: number
}
...
function LayerRow({ capa, estados, onChange, onRemove, canRemove, index }: LayerRowProps) {
  ...
  return (
    <div className="border border-gray-200 rounded-xl p-3 bg-white shadow-sm space-y-2.5">
      ...
      <input
        type="text" placeholder="Negocio a buscar (ej: little caesars)" value={capa.keyword}
        onChange={e => set('keyword', e.target.value)}
        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm placeholder-gray-400 focus:ring-2 focus:ring-brand-copper focus:border-brand-copper outline-none"
      />
      <input
        type="text" placeholder="Nombre en el reporte (ej: Little Caesars)" value={capa.label}
        onChange={e => set('label', e.target.value)}
        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm placeholder-gray-400 focus:ring-2 focus:ring-brand-copper focus:border-brand-copper outline-none"
      />
      <select
        value={capa.estado} onChange={e => set('estado', e.target.value)}
        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-brand-copper focus:border-brand-copper outline-none bg-white"
      >
        {estados.map(est => <option key={est.clave} value={est.clave}>{est.clave} · {est.nombre}</option>)}
      </select>
      <div className="flex items-center justify-between">
        <div className="flex gap-1">
          {COLORES.map(c => (
            <button key={c.value} title={c.label} onClick={() => set('color', c.value)}
              ...
              className={`w-5 h-5 rounded-full transition-transform border-2 ${capa.color === c.value ? 'scale-125 border-gray-700' : 'border-transparent hover:scale-110'}`}
            />
          ))}
        </div>
        <div className="flex gap-3 text-xs">
          {(['circle', 'star'] as const).map(ico => (
            <label key={ico} className="flex items-center gap-1 cursor-pointer select-none">
              <input type="radio" name={`icon_${capa.id}`} value={ico} checked={capa.icon === ico}
                onChange={() => set('icon', ico)} className="accent-brand-navy" />
...
export default function WizardCompetenciaPage() {
  ...
  useEffect(() => {
    apiEstados().then((data: EstadoCatalogo[]) => setEstados(data)).catch(console.error)
    apiBdStatus()
  ...
  const buildPayload = (etl: boolean) => ({
    ...
    polygon,
    capas:                   capas.map(c => ({
      keyword: c.keyword.trim(), label: c.label.trim() || c.keyword.trim(),
      color: c.color, icon: c.icon, estado: c.estado,
    })),
    formato,
  ...
  const handleGenerar = async () => {
    if (!polygon) { setStatus('Selecciona un estado y municipio primero.'); setStatusType('error'); return }
    if (capas.some(c => !c.keyword.trim())) { setStatus('Todas las capas deben tener un keyword.'); setStatusType('error'); return }
    ...
    try {
      ...
      setStatus(`${total} establecimiento${total !== 1 ? 's' : ''} encontrado${total !== 1 ? 's' : ''}. ${preview.resumen.total_zonas} zonas analizadas. Generando archivo...`)
      setStatusType('ok')
      ...
      <div className="flex flex-col flex-1 overflow-hidden min-w-0">
        ...
        <div className="flex flex-1 overflow-hidden min-h-0">
            ...
            {bdInfo && (
              <div className="px-4 pt-3 pb-2 border-b border-gray-100 flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="text-gray-400 hover:text-brand-navy transition-colors flex-shrink-0"
            ...
            <div className="flex-1 overflow-y-auto sidebar-scroll px-4 py-5 space-y-6">
              ...
              <section>
                <div className="flex items-center gap-2 mb-2">
                  <span className="w-5 h-5 rounded-full bg-brand-navy text-white text-xs flex items-center justify-center font-bold flex-shrink-0">1</span>
                  <h2 className="text-sm font-semibold text-gray-700">Área de análisis</h2>
                </div>

                <div className="space-y-2 mb-2">
                  <select
                    value={selectedEnt} onChange={e => handleEstadoChange(e.target.value)}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-brand-copper focus:border-brand-copper outline-none bg-white"
                  >
                    <option value="">— Selecciona un estado —</option>
                    {estados.map(est => (
                      <option key={est.clave} value={est.clave}>{est.clave} · {est.nombre}</option>
                    ))}
                  </select>
                  ...
                  {selectedEnt && (
                    <div className="relative">
                      <select
                        value={selectedMun} onChange={e => handleMunicipioChange(e.target.value)}
                        disabled={munLoading}
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-brand-copper focus:border-brand-copper outline-none bg-white disabled:opacity-60"
                      >
                        <option value="">— Selecciona un municipio —</option>
                        {municipios.map(m => <option key={m.clave} value={m.clave}>{m.nombre}</option>)}
                      </select>
                      {munLoading && (
                        <span className="absolute right-3 top-2.5 w-4 h-4 border-2 border-brand-copper border-t-transparent rounded-full animate-spin" />
                      )}
                    </div>
                  )}

                  {selectedEnt && municipios.length === 0 && !munLoading && (
                    <p className="text-xs text-gray-400 px-1">No hay municipios cargados para este estado.</p>
                  )}
                </div>
                ...
                <div className={`text-xs rounded-lg px-3 py-2.5 border flex items-center gap-2 ${
                  polygon ? 'bg-green-50 border-green-200 text-green-700' : 'bg-amber-50 border-amber-200 text-amber-700'
                }`}>
                  {polygon ? (
                    <>
                    ...
                    <>
                      ...
                      Selecciona un estado y municipio
                    </>
                  )}
                </div>
              </section>
              ...
              <section>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-brand-navy text-white text-xs flex items-center justify-center font-bold flex-shrink-0">2</span>
                    <h2 className="text-sm font-semibold text-gray-700">Capas de búsqueda</h2>
                  </div>
                  <button onClick={addCapa} disabled={capas.length >= 8}
                    className="text-xs text-brand-navy hover:text-brand-copper font-medium disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1"
                  >
                ...
                <div className="space-y-2">
                  {capas.map((capa, i) => (
                    <LayerRow key={capa.id} capa={capa} estados={estados}
                      onChange={updated => updateCapa(capa.id, updated)}
                      onRemove={() => removeCapa(capa.id)}
                      canRemove={capas.length > 1} index={i}
                    />
                  ))}
                </div>
              </section>
              ...
              <section>
                <div className="flex items-center gap-2 mb-3">
                  <span className="w-5 h-5 rounded-full bg-brand-navy text-white text-xs flex items-center justify-center font-bold flex-shrink-0">3</span>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {([
                      ...
                      <label key={n.value} className={`flex flex-col gap-0.5 p-2.5 rounded-lg border cursor-pointer transition-colors ${
                        nivelGeografico === n.value ? 'bg-brand-bg border-brand-copper' : 'border-gray-100 hover:bg-gray-50'
                      }`}>
                        <div className="flex items-center gap-2">
                  ...
                  <div className="space-y-1.5">
                    {[
                      ...
                      <label key={opt.value} className={`flex items-start gap-2.5 p-2.5 rounded-lg border cursor-pointer transition-colors ${
                        clasificacion === opt.value ? 'bg-brand-bg border-brand-copper/60' : 'border-gray-100 hover:bg-gray-50'
                      }`}>
                        <input type="radio" name="clasificacion" value={opt.value}
                          checked={clasificacion === opt.value}
                          onChange={() => setClasificacion(opt.value as Clasificacion)}
                          className="accent-brand-navy mt-0.5 flex-shrink-0"
                        />
                        <div>
                  ...
                  <div className="flex gap-3">
                    {([
                      ...
                      <label key={f.value} className={`flex-1 flex items-center gap-2 p-2.5 rounded-lg border cursor-pointer transition-colors ${
                        formato === f.value ? 'bg-brand-bg border-brand-copper/60' : 'border-gray-100 hover:bg-gray-50'
                      }`}>
              ...
              <div className="flex gap-2">
                {previewData && (
                  <button
                    ...
                    className="flex-shrink-0 border border-brand-copper text-brand-copper rounded-xl py-3 px-3 text-sm font-semibold hover:bg-brand-bg transition-colors"
                    title="Ver KPIs"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <button
                  ...
                  className="flex-1 bg-brand-navy text-white rounded-xl py-3 font-semibold text-sm hover:bg-brand-green active:bg-brand-black disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2 shadow-sm"
                >
                  {loading ? (
                    <>
          ...
          <main className="flex-1 relative overflow-hidden">
            <MapContainer center={[23.6345, -102.5528]} zoom={5} className="h-full w-full" zoomControl>
              <TileLayer
            ...
            {!polygon && (
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-white rounded-full shadow-lg px-5 py-2.5 text-sm text-gray-600 flex items-center gap-2 pointer-events-none z-[1000]">
                <svg className="w-4 h-4 text-brand-copper" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
            ...
            {loading && (
              <div className="absolute inset-0 bg-black/20 flex items-center justify-center z-[999] pointer-events-none">
                <div className="bg-white rounded-2xl shadow-xl px-8 py-6 flex flex-col items-center gap-3">
                  <span className="w-10 h-10 border-4 border-brand-navy border-t-transparent rounded-full animate-spin" />
          ...
          {/* ── PANEL KPIs ───────────────────────────────────────────────── */}
          <div className={`flex-shrink-0 border-l border-gray-200 bg-[#0d1b2a] transition-all duration-300 overflow-hidden ${showKPI && previewData ? 'w-80' : 'w-0'}`}>
            {showKPI && previewData && (
