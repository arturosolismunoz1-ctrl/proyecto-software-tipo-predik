...
function WidgetShell({ title, icon, editMode, children, className = '' }: WidgetShellProps) {
  return (
    <div className={`bg-white rounded-xl overflow-hidden flex flex-col border ${
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-brand-beige bg-brand-navy/5 flex-shrink-0">
        {editMode && (
          <span className="text-brand-copper select-none cursor-default">
  ...
  return (
    <div className="h-full flex flex-col p-2">
      <div className="flex-1 overflow-y-auto">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-brand-black/30 py-6">
  ...
  return (
    <div className="h-full flex flex-col gap-3">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-12 grid-rows-2 gap-3">
        {/* Mapa — 7/12 columnas, fila 1 */}
        <div className="col-span-7 row-span-1" style={{ minHeight: 280 }}>
          <WidgetShell title="Mapa" icon="map" editMode={editMode} className="h-full">
            <WidgetMapa />
          </WidgetShell>
        </div>

        {/* Gráfica — 5/12 columnas, fila 1 */}
        <div className="col-span-5 row-span-1" style={{ minHeight: 280 }}>
          <WidgetShell title="Actividad mensual" icon="chart-bar" editMode={editMode} className="h-full">
            <WidgetGrafica analisisList={analisisList} />
          </WidgetShell>
        </div>

        {/* Proyectos — 6/12 columnas, fila 2 */}
        <div className="col-span-6 row-span-1" style={{ minHeight: 240 }}>
          <WidgetShell title="Análisis recientes" icon="clock" editMode={editMode} className="h-full">
            <WidgetProyectos analisisList={analisisList} />
          </WidgetShell>
        </div>

        {/* NSE — 6/12 columnas, fila 2 */}
        <div className="col-span-6 row-span-1" style={{ minHeight: 240 }}>
          <WidgetShell title="Distribución NSE" icon="chart-pie" editMode={editMode} className="h-full">
            <WidgetNSE />
          </WidgetShell>
        </div>
      </div>
